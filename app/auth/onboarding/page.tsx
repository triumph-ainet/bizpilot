...
export default function OnboardingPage() {
  ...
  const [extracted, setExtracted] = useState<{ name: string; estimated_price: number | null; quantity: number | null } | null>(null)
    ...
    <div className="min-h-screen bg-cream">
      ...
      <div className="bg-green px-6 pt-14 pb-6">
        ...
        <h1 className="font-fraunces text-[26px] font-black text-white leading-tight">
          {step === 0 && <>Tell us about<br />your <em className="text-amber not-italic">business</em></>}
          {step === 1 && <>Add your<br />first <em className="text-amber not-italic">product</em></>}
          {step === 2 && <>Where should<br />we send <em className="text-amber not-italic">payments?</em></>}
          {step === 3 && <>You're all<br /><em className="text-amber not-italic">set!</em> 🎉</>}
        </h1>
      ...
      <div className="px-6 py-6 animate-fade-up">
        ...
        {step === 1 && (
          <div className="space-y-4">
            ...
            <button onClick={() => setStep(2)} className="w-full text-center text-sm text-ink-light py-2">
              Skip for now →
            </button>
          </div>
        )}
            ...
            <div className="bg-white rounded-2xl p-5 shadow-card space-y-4">
              ...
              <div className="flex flex-col gap-2">
                ...
                {accountNumber.length === 10 && bankCode && !verifiedName && (
                  <Button size="sm" variant="outline" loading={verifying} onClick={handleVerifyAccount} className="mt-1">
                    Verify Account
                  </Button>
                )}
              </div>
