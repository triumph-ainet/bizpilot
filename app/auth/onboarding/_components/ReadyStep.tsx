  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-2xl p-5 shadow-card space-y-3">
        ...
        {[
          { icon: <MessageCircle className="w-5 h-5" />, text: 'Customer sends you an order' },
          { icon: <Zap className="w-5 h-5" />, text: 'AI replies and sends payment link' },
          {
            icon: <CheckCircle className="w-5 h-5" />,
            text: 'Payment confirmed, inventory updated',
          },
        ].map((item) => (
      ...
      <Button variant="amber" loading={loading} onClick={onFinish}>
        Go to My Dashboard →
      </Button>
    </div>
  );
