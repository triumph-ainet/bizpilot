...
export default async function AnalyticsPage() {
  ...
  try {
    ...
    if (cached && Date.now() - cached.ts < 1000 * 60 * 5) {
      ...
      aiInsights = (raw || []).map((r: any, i: number) => ({
        title: r.name || `Suggestion ${i + 1}`,
        detail: r.short_reason || '',
      }));
  ...
  const todayRevenue = orders
    .filter((o) => o.status === 'paid' && o.created_at && o.created_at.startsWith(todayKey))
    .reduce((s, o) => s + Number(o.total || 0), 0);
  const yesterdayRevenue = orders
    .filter((o) => o.status === 'paid' && o.created_at && o.created_at.startsWith(yesterday))
    .reduce((s, o) => s + Number(o.total || 0), 0);
  const revenueChange =
    yesterdayRevenue === 0
      ? 0
      : Math.round(((todayRevenue - yesterdayRevenue) / Math.max(1, yesterdayRevenue)) * 100);
  ...
  return (
    <div className="min-h-screen bg-cream pb-24">
      ...
      <BottomNav active="/vendor/dashboard/analytics" />
    </div>
  );
}
