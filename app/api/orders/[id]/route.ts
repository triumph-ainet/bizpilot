...
export async function PATCH(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const { sendNotificationEmail, sendInvoiceEmail } =
    ...
    const sessionUrl = session?.token
      ? `${process.env.NEXT_PUBLIC_APP_URL}/session/${session.token}`
      : null;

    const recipient =
      session?.customer_email ||
      (order.customer_identifier && order.customer_identifier.includes('@')
    ...
    if (recipient) {
      const vendor = await supabase
        ...
        .eq('id', session?.vendor_id || order.vendor_id)
        .single();
      ...
      await sendNotificationEmail(
        ...
        order.customer_identifier,
        msg,
        sessionUrl ? `<p><a href="${sessionUrl}">Open your order</a></p>` : undefined
      ).catch(() => null);
    }
  } catch (e) {
